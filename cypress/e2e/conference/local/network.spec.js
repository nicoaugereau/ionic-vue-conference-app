...
describe("Mock et erreurs réseau", () => {
  ...
  it("Le site est en ligne", () => {
    cy.environnement(environment).then((url) => {
      sessionStorage.setItem("url", url.frontendUrl);
  ...
  it("Le site est en ligne mais avec des lenteurs réseau", () => {
    ...
    cy.visit(sessionStorage.getItem("url"));
  ...
  it("Simuler une erreur réseau", () => {
    cy.intercept("GET", "/data/*", { forceNetworkError: true }).as(
      "getNetworkFailure"
    );
  ...
  it("Mock des données Schedules et Speakers", () => {
    // Schedule Tab
    cy.intercept("GET", "/data/sessions*", {
      fixture: "mock-sessions.json",
    }).as("mockSessionList");
    cy.intercept("GET", "/data/speakers*", {
      fixture: "mock-speakers.json",
    }).as("mockSpeakersList");
    cy.visit(sessionStorage.getItem("url"));
