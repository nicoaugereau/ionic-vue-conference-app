...
const getStateTimeSpan = reports => {
    ...
    const maxSpan = spans.reduce((currentMaxSpan, span) => {
        const start = new Date(Math.min(currentMaxSpan.start.getTime(), span.start.getTime()))
        const end = new Date(Math.max(currentMaxSpan.end.getTime(), span.end.getTime()))
        return { start, end }
    })
...
function pugToHtml(data, output, options) {
    ...
    var testsListName = 'Liste des tests en échec'
    ...
    let chart_passed = `#26b526 0, #26b526 ${pass_percent}%,`
    let chart_failed = `#fa0f36 0, #fa0f36 ${(Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent))).toFixed(4)}%,`
    // skipped -> pending
    let chart_pending = `#99ddff 0, #99ddff ${(Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent)) + Number(parseFloat(skip_percent))).toFixed(4)}%,`
    // pendinf -> skipped
    let chart_skipped = `#666 0, #666 ${(
        Number(parseFloat(pass_percent)) +
        Number(parseFloat(fail_percent)) +
        Number(parseFloat(skip_percent)) +
        Number(parseFloat(pending_percent))
    ).toFixed(4)}%`
