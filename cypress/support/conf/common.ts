...
export const tab = (options: any) => {
  if (options.title) {
    let d: any = tabs.translate.find((p) => p.title === options.title);
    cy.get(`[data-testid="tab-bar-${d.id}"] ion-label`).should(
      "have.text",
      d.tab
    );
  }
  if (options.tab) {
    let d: any = tabs.translate.find((p) => p.title === options.tab);
...
export const conference = (options: any) => {
  if (options.conf) {
    sessionStorage.setItem("conference", options.conf);
    cy.get(".searchbar-input").clear();
    cy.get(".searchbar-input").type(options.conf);
  }
  if (options.control) {
    if (options.title)
      cy.get("ion-item ion-label h3").should("have.text", options.title);
...
export const conferencier = (options: any) => {
  if (options.control)
    cy.get('[data-testid="tabs"] ion-toolbar > ion-title.title-md', {
      timeout: msTimeout,
    }).should("have.text", options.profil);
  if (options.speaker) {
    sessionStorage.setItem("conferencier", options.speaker);
